...
void QCustomRuler::AnalyseSound(cVideoFile *MusicObject) {
    IsAnalysed=MusicObject->LoadAnalyseSound(&Peak,&Moyenne);
    if (IsAnalysed) {
        PrepareSoundWave();
        Analysed=100;
    } else {
        qint64          NewPosition=0,Position=-1;
        qint64          Duration=QTime(0,0,0,0).msecsTo(MusicObject->Duration);
        int             WantedValues;
        QList<qreal>    Values;
        ...
        //*******************************************************************************************
        while ((!IsAnalysed)&&(Position!=NewPosition)) {
            Analysed=qreal(NewPosition)/qreal(Duration);
            Position=NewPosition;
            MusicObject->ReadFrame(true,Position*1000,true,false,&AnalyseMusic,1,true);
        ...
        IsAnalysed=true;
        Analysed=100;
        MusicObject->SaveAnalyseSound(&Peak,&Moyenne);
    }
}
